...
from functions.svd_ddnm import ddnm_diffusion, ddnm_plus_diffusion, new_diffusion, dps_diffusion, pigdm_diffusion
...
class Diffusion(object):
    ...
    def svd_based_ddnm_plus(self, model, cls_fn):
        ...
        if deg == 'cs_walshhadamard':
            ...
            loaded = np.ones([256, 256])
            loaded[128-64:128+64, 128-64:128+64] = 0
        ...
        for x_orig, classes in pbar:
            ...
            for t in ["input", "recon", "label"]:
                (self.args.image_folder / t).mkdir(exist_ok=True)
            for i in range(len(Apy)):
                tvu.save_image(
                    inverse_data_transform(config, Apy[i]),
                    os.path.join(self.args.image_folder, f"input/{idx_so_far + i}.png")
                )
                tvu.save_image(
                    inverse_data_transform(config, x_orig[i]),
                    os.path.join(self.args.image_folder, f"label/{idx_so_far + i}.png")
                )
            ...
            # total should be fixed to 1000
            alpha_schedule = get_alpha_schedule(type=config.mcg.schedule, 
                                                start=config.mcg.alpha_start, 
                                                end=config.mcg.alpha_end,
                                                total=1000)
            if args.method == "new":
                x, _ = new_diffusion(x, model, self.betas, self.args.eta, A_funcs, y, cls_fn=cls_fn, classes=classes, config=config,
                ...
                x, _ = dps_diffusion(x, model, self.betas, self.args.eta, A_funcs, y, cls_fn=cls_fn, classes=classes, config=config,
                                        alpha_schedule=alpha_schedule)
            elif args.method == "pigdm":
                x, _ = pigdm_diffusion(x, model, self.betas, self.args.eta, A_funcs, y, cls_fn=cls_fn, classes=classes, config=config)
            elif args.method == "modi":
            ...
            for j in range(x[0].size(0)):
                tvu.save_image(
                    x[0][j], os.path.join(self.args.image_folder, f"recon/{idx_so_far + j}.png")
                )
                ...
                # compute lpips: scale to range [-1, 1]
                orig = (orig * 2. - 1.).to("cuda")
                recon = (x[0][j] * 2. - 1.).to("cuda")
                d = loss_fn_vgg(orig, recon.view(1, 3, 256, 256))
                avg_lpips += d.item()

            idx_so_far += y.shape[0]

            pbar.set_description(f"PSNR: {(avg_psnr / (idx_so_far - idx_init)):.2f}  /  LPIPS: {(avg_lpips / (idx_so_far - idx_init)):.3f}")

        avg_psnr = avg_psnr / (idx_so_far - idx_init)
        avg_lpips = avg_lpips / (idx_so_far - idx_init)
        print("Total Average PSNR: %.2f" % avg_psnr)
        print("Total Average LPIPS: %.3f" % avg_lpips)
        print("Number of samples: %d" % (idx_so_far - idx_init))

        # save experiment parameters / results as json
        summary = {}
        summary["params"] = {
            "T": self.args.T_sampling,
            "clip_x0t": self.config.clip.x0t,
            "clip_x0that": self.config.clip.x0that,
            "projection_type": self.config.projection_type,
            "CG_iter": self.config.CG_iter}
        summary["alpha_schedule"] = {
            "alpha_start": self.config.mcg.alpha_start, 
            "alpha_end": self.config.mcg.alpha_end,
            "schedule": self.config.mcg.schedule
            }
        summary["results"] = {"PSNR": avg_psnr, "LPIPS": avg_lpips}
